...
sample_names <- sub("*_comp_hOH7_KSHV_hybrids_ua.viennad.line", "", vfiles)
classes <- c("5mi3m", "5v3m", "5m3mi", "5m3v")

Merger <- function(file,samp){

vienna <- read.delim(file,header=F, stringsAsFactors = F)
kshv <- c(grep("kshv-miR", vienna$V1))
vienna <- cSplit(vienna,1,"_")
vienna <- select(vienna, c(14,1,2,6,8,9,10,11))
vienna <- rbindlist(list(subset.data.frame(vienna[-kshv,], select=c(1,2,3,4,7,8)),
